...
`package.json` の version を書き換え、型と試験を通してから Firefox 向けに組み、`release/` へ 2 つの zip を置きます。
`xpic2-<version>-firefox.zip` が提出する拡張機能、`xpic2-<version>-source.zip` が審査へ添える原文です。
縮めたコードを載せた拡張機能には原文の同梱が要るので、この 2 つは対で作ります。

原文に詰めるのは git が追っているファイルだけです。
作業ツリーの側を詰めるため、コミット前の変更も組んだ `dist/` と揃った状態で入ります。
...
- 設定の「カラム数」のスライダーで列数を変えます（左端の「自動」は画面の幅から決めます）。
- 画像 / 動画 / GIF / RT のチェックで取得対象を絞ります。切り替えると読み直します。
- 「複数画像を分割」を切ると、1 投稿を 1 タイルにまとめ、画像を左から横並びにします。
- タイルの操作バーの右端（「この投稿を消す」）を押すと、そのポストが壁から外れます。記録はブラウザに残るので、以後はどの取得元でも出ません。
- 投稿を消した直後は、画面下の「元に戻す」で直前の 1 件だけを戻せます。
- 壁では矢印キーでタイルを選び、`Enter` で拡大、`l` でいいね、`b` でブックマークを切り替え、`m` で選択中の投稿を消し、`r` で再読み込みします。
- タイルをクリックで拡大表示、`←` `→` で移動、`Esc` で閉じます。
- `/` で検索欄にフォーカスします。
- `?` でキーボードショートカットの一覧を開きます。
...
- 広告（プロモーション）は表示しません。タイムラインのエントリに付く `promotedMetadata` で見分けるので、本文や作者による推測は挟みません。おすすめ・検索・フォロー中に混ざる分がここで落ちます。
- リポストは元ポストとして扱い、リポストした人を添えて表示します。引用ポストの引用元の画像は取り込みません。
- いいね・リポスト・ブックマークは自分のアカウントに実際に反映されます。取り消しも同じ画面から行えます。
- 消した投稿の記録はこのブラウザの localStorage だけに置き、5000 件を超えると古いものから落とします。戻すのは設定の「記録を消して戻す」でまとめて行う形だけです。
