// 拾い終えた投稿を、少し置いてから壁から外す。
//
// おすすめとフォロー中は、流れてくるものを眺めながら気に入ったものを拾う場になる。
// 拾ったものがそのまま残っていると、次に読み込んだぶんと混ざり、どこまで見たのかが判らなくなる。
// かといって押した瞬間に消すと、押し間違いを戻す手が無い。
// そこで、いいねかブックマークを付けてから少し置き、そのあいだに取り消せば残るようにする。
//
// 消す対象はポストであってメディアではない。1 投稿を複数のタイルに分けていても、まとめて外れる。
 ...
 * 返事待ちのあいだも数えない。楽観更新の印は送信が失敗すれば押す前へ戻るので、
 * 待たずに数え始めると、猶予が過ぎた後で失敗した投稿まで壁から外したままになる。
 */
